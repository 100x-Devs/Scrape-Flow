        ...
        for(const CurrentNode of nodes){
            ...
            if(invalidInputs.length > 0){
                ...
                if(incomers.every(incomer => planned.has(incomer.id))){
                    ...
                    // this means that particular node has an invalid input
...
function getInvalidInputs(node: AppNode, edges: Edge[], planned: Set<string>){
    ...
    for(const input of inputs){
        ...
        if(requiredInputProvidedByVisitedOuput){
            ...
            // privided by a task that is already planned 

            continue;
        }else if(!input.required){
            // if the input is not required but there is an output linked to it
            // then if need to be sure that the output is already planned
